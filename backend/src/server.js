...
function validateRazorpayConfig() {
  ...
  if (!keyId.startsWith("rzp_test_") && !keyId.startsWith("rzp_live_")) {
    ...
    console.error(`   Key ID should start with 'rzp_test_' or 'rzp_live_'`);
    console.error(`   Current: ${keyId.substring(0, 15)}...\n`);
    return false;
  }

  if (keySecret.length < 20) {
    console.error("❌ KEY SECRET TOO SHORT!");
    console.error(`   Key Secret should be at least 20 characters`);
    console.error(`   Current length: ${keySecret.length}\n`);
    return false;
  }

  const mode = keyId.startsWith("rzp_test_") ? "TEST" : "LIVE";
  console.log(`✅ Razorpay ${mode} mode configured`);
  console.log(`   Key ID: ${keyId.substring(0, 15)}...`);
  console.log(
    `   Secret: ${keySecret.substring(0, 5)}...${keySecret.substring(keySecret.length - 5)}\n`,
  );

...
router.post("/sync-calendar", protect, async (req, res) => {
    try {
        if (!req.user.leetcodeUsername) {
            return res.status(400).json({ error: "LeetCode username not set on your profile" });
        }

        console.log(`Starting sync for user ${req.user._id} with username ${req.user.leetcodeUsername}`);
